...
/**
 * 返回解析配置数据后的虚拟`DOM`字符串 最终放在`vm.options._render`
 */
export function compileToFunc(template) {
    // `ast`描述编译模板类型 内容等 尚未对模板语法解析 如 `{{ value }}`
...
function genCode(ast) {
    ...
    return `_createEl('${tagName}', ${attrs?.length > 0 ? genAttrs(attrs) : null}${children?.length ? `,${_children}` : ''})`;
}
...
function genChild(node) {
    if (node.type === EL_TYPE) {
        ...
        if (!exprReg.test(text)) {
            ...
            // _createTxt(_createStr(name) + 'hello' + _createStr(age))
            let tokens = [];
            let match;
            exprReg.lastIndex = 0;
            let lastIndex = 0;
            ...
            while (match = exprReg.exec(text)) {
                ...
                tokens.push(`_createStr(${match[1].trim()})`);
